#!/bin/sh
# run.sh — runs the built LED tester on an emulated Amiga 500 with a fake RBCP
...
#
# The tester reads every LED once a field but changes nothing on its own, so a
# run that presses nothing shows the screen it drew at startup and no more.
# The default RBCP_KEYS steps a mode, a colour, a brightness, a period and a
# hold, then moves to the next LED and steps its mode.  Set it to press
# something else.
#
...
lua="$here/rbcp_dev_led.lua"

: "${RBCP_TARGET:=256}"
: "${RBCP_KEYS:=240:M;300:C;360:B;420:P;480:H;540:Cursor Down;600:M}"
export RBCP_KEYS
...
image="$build/amiga_led_test.bin"
...
# The tester goes in the Kickstart socket.  It runs from reset and never hands
