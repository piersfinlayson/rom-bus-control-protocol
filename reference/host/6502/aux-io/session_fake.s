; session_fake.s — a session with no device at the other end
; Copyright (C) 2026 Piers Finlayson <piers@piers.rocks>
;
; Provides session.s's interface out of fixed answers, so that the demo build
; can be looked at under an emulator, where there is nothing to knock on and
; nothing past the knock can run.  It is never linked into a binary that talks
; to hardware.
;
; What it does not model: the knock, the command encoding, the polling
; sequence and every way a real device can refuse.  None of those are visible
; on screen, which is the whole reason this file can stand in.
