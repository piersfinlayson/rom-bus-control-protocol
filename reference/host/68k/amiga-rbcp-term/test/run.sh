#!/bin/sh
# run.sh — runs the built terminal on an emulated Amiga 500 with a fake RBCP
...
#
# A terminal with nobody at it types nothing and the far end has nothing to
# answer, so this types a line unless the caller says what to type.
#
...
lua="$here/rbcp_dev_term.lua"
font="$here/../../amiga-common/font_8x8.bin"
...
# The screen is a bitmap and the font the image was built with turns it back
# into lines of text at the end of a run.
[ -f "$font" ] || { echo "no $font" >&2; exit 1; }
RBCP_FONT=$(cd "$(dirname "$font")" && pwd)/$(basename "$font")
export RBCP_FONT
...
# The terminal goes in the Kickstart socket.  It runs from reset and never
# hands the machine back, so there is no second image and nothing to switch to.
...
# dump it expects.  The terminal is in the Kickstart socket, so it never
