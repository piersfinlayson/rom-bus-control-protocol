#!/bin/sh
# run.sh — runs the built meter on an emulated Amiga 500 with a fake RBCP
...
#
# The meter counts against chip DMA on and against chip DMA off, and only `S`
# moves it from one to the other.  A run that presses nothing leaves the second
# pair of counters at zero, so this presses `S` partway through unless the
# caller says where.
#
...
# The fake device lives in the bootloader's harness and serves both
# applications.
lua="$here/../../amiga-boot/test/rbcp_dev_amiga.lua"
...
# The meter goes in the Kickstart socket.  It runs from reset and never hands
# the machine back, so there is no second image and nothing to switch to.
...
# dump it expects.  The meter is in the Kickstart socket, so it never matches,
