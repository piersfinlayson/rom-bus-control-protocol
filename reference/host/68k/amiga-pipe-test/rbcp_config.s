; rbcp_config.s — RBCP configuration for the Amiga pipe throughput test
...
;
; CONFIG_RBCP_TIMEOUT_RETRIES is zero.  A retried command is a command the
; tester sent twice and counted once, and the bytes it carried reach the far
; end twice.  The stream has to be what the counts say it is.
; ---------------------------------------------------------------------------
CONFIG_RBCP_POLL_TIMEOUT    EQU $0000FFFF
CONFIG_RBCP_NV_POLL_TIMEOUT EQU $00FFFFFF       ; flash erase takes ms
CONFIG_RBCP_TIMEOUT_RETRIES EQU 0
CONFIG_RBCP_CMD_PAUSE       EQU $100            ; the pause between reset stages
...
; transposition.  The longest thing the tester reads is a 24-byte device name,
; so 48 bytes is twice what it needs.  It sits in the application variable
; area, so the 68000 reaches it with absolute short addressing — see the chip
; RAM layout in amiga_defs.s.
; ---------------------------------------------------------------------------
