...
;
; The tester is a ROM.  It owns the machine from reset, it never gives it back,
; and you switch off when you have read the figure.  So there is no exit to
; repair, and none of the apparatus an exit needs — the image checksum, the
...
; three strings the device calls itself, and the two questions this tester has
; that no other host asks: whether there is a pipe at all, and whether pipe 0
; takes bytes from the host.
;
; Everything here runs with interrupts masked and the tester executing from
